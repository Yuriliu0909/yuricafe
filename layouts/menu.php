<div>
    <!-- Bootstrap core CSS -->
    <link href="../assets/dist/css/bootstrap.min.css" rel="stylesheet">
        <div class="bg-dark py-5 container px-4 px-lg-5 my-5">
            <div class="text-center text-white">
                <h1 class="display-4 fw-bolder">Store</h1>
                <p class="lead fw-normal text-white-50 mb-0">pick love</p>
            </div>
        </div>
    <!-- Section-->
    <section class="py-5">
        <div class="container px-4 px-lg-5 mt-5">
            <div id="products-view" class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
                <div class="col mb-5">
                    <div class="card h-100">
                        <img class="card-img-top" src="../images/scone.jpg" alt="..."</img>
                        <!-- Product details-->
                        <div class="card-body p-4">
                            <div class="text-center" id="lattee name">
                                <!-- Product name default-->
                                <h5 class="fw-bolder" id="lattee price">Fancy Product</h5>
                                <!-- Product price default-->
                                $40.00 - $80.00
                            </div>
                            <form action="/action_page.php">
                                <label for="quantity">quantity:</label><br>
                                <input type="text" id="quantity" value="1"><br><br>
                            </form>
                        </div>
                        <!-- Product actions-->
                        <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                            <div class="text-center"><button class="btn btn-outline-dark mt-auto" >Add to Cart</button></div>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        <script src="../scripts/menu.js"></script>
        </div>
    </section>

